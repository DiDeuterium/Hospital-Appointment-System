...
系统增加两个只读查询视图，用于减少后端常见联表查询的重复 SQL：

```text
v_appointment_detail    预约详情视图
v_available_schedule    可预约排班视图
```

`v_appointment_detail` 汇总预约、患者、排班、医生、科室和支付记录信息，适合以下查询场景：
...
`v_available_schedule` 只返回可预约排班，过滤条件包括：

```text
schedule.status = 1
doctor.status = 1
department.status = 1
schedule.rest_quota > 0
```
...
这两个视图只用于读场景。新增预约、取消预约、模拟支付、排班审核等写操作仍然直接操作原始业务表，并由后端事务保证数据一致性。
